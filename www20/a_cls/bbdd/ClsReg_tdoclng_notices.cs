...
namespace testudines.cls.bbdd
{
    ...
    //<//summary>
    public class ClsReg_tdoclng_notices
    {
        private cls.bbdd.ClsReg_tdoc oRegDoc = new ClsReg_tdoc();
        private bool m_ErrorBoolean = false;
        ...
        private DateTime m_DatePublication = DateTime.Now;
        ...
        //---------------------------------------------------
        //public ClsReg_tdoclng_notices(string szSqlConnectionString)
        //{
        public ClsReg_tdoclng_notices()
        {
        ...
        public void FncClear()
        {
            m_DocId = 0;
            m_DocLngId = "";
            m_DatePublication = DateTime.Now;
            m_Title = "";
            m_Abstract = "";
            m_Body = "";
        }
        ...
        public void FncSqlBuildCommands()
        {
            ...
            //-------------- COMANDO SELECT  ---------------------------------------
            //----------------------------------------------------------------------
            szSql = "SELECT * FROM tdoclng_notices  ";
            szSql += " WHERE ";
            szSql += "(DocId=@DocId )";
            ...
            //-------------- COMANDO INSERT ----------------------------------------
            //----------------------------------------------------------------------
            szSql = "INSERT INTO tdoclng_notices";

            szSql += "(";

            szSql += " DocId ";
            szSql += ", DocLngId ";
            szSql += ", DatePublication ";
            szSql += ", Title ";
            szSql += ", Abstract ";
            szSql += ", Body ";
            szSql += " ) VALUES     (";
            ...
            oSqlCmdInsert.Parameters.Add("@DatePublication", MySql.Data.MySqlClient.MySqlDbType.Date);
            ...
            //-------------- COMANDO UPDATE ---------------------------------------
            //----------------------------------------------------------------------
            szSql = "UPDATE tdoclng_notices SET ";

            szSql += "DatePublication=@DatePublication ";
            szSql += ", Title=@Title ";
            ...
            oSqlCmdUpdate.Parameters.Add("@DatePublication", MySql.Data.MySqlClient.MySqlDbType.Date);
            ...
            //-------------- COMANDO DELETE  ---------------------------------------
            //----------------------------------------------------------------------
            szSql = "DELETE FROM tdoclng_notices  ";
            szSql += " WHERE ";
            szSql += "(DocId=@DocId )";
            szSql += " AND (DocLngId=@DocLngId )";
        ...
        {
            string m_urlTemplate = "/notices/notice/";
            ...
            UInt16 iCount = 20;
            FncCache_GET_lastImgBld(ref iCount);
        ...
        private bool FncSqlUpdate()
        {
            ...
            oSqlCmdUpdate.Parameters["@DatePublication"].Value = m_DatePublication;
        ...
        private bool FncSqlInsert()
        {
            ...
            oSqlCmdInsert.Parameters["@DatePublication"].Value = m_DatePublication;
        ...
        public bool FncSqlFind(UInt64 DocId, String DocLngId)
        {
            ...
            try
            {
                ...
                if ((oDR.HasRows) && (oDR.Read()))
                {
                    ...
                    m_DatePublication = Convert.ToDateTime(oDR["DatePublication"].ToString().Trim());
...

        {
            ...
            szHtml += "<div class=\"divboxArticle\"><h2>" + Resources.Strings.Notice + "</h2>" + m_Body + "</div>\n";
        ...
        {
            get { return m_ErrorBoolean; }
            set { m_ErrorBoolean = value; }
        }
        //------------------------------
        ...
        public String DocLngId
        {
            set
            {
                ...
                if (sz.Length > 2)
                {
                    sz = sz.Substring(0, 1);
                    m_ErrorBoolean = true;
                    m_ErrorString = "Trimmed m_DocLngId because is it too long,MaxLength=2";
                }
        ...
        public String Title
        {
            set
            {
                ...
                if (sz.Length > 256)
                {
                    ...
                    m_ErrorBoolean = true;
                    m_ErrorString = "Trimmed m_Title because is it too long,MaxLength=256";
                }
                m_Title = sz;
        ...
        //-------------------------------------------------
        public String FncHtmlBreadCrumb(bool bRebuild)
        {
            
            string szFilePathName =cls.cache.ClsCache.FncCacheFilePath(m_DocId, m_DocLngId, "notice_BC");
            if (bRebuild) cls.ClsUtils.FncPathFileDelete(szFilePathName);
            if (cls.ClsUtils.FncPathFileExist(szFilePathName))
                {
                return cls.cache.ClsCache.FncCacheFileRead(szFilePathName);
            }
            else
            {
                String szHtml = FncHtmlBldCacheBreadCrumb();
                cls.cache.ClsCache.FncCacheFileSave(ref szFilePathName, ref szHtml);
                return szHtml;
            }
        ...
        private string FncHtmlBldCacheBreadCrumb()
        {
            ...
            szDescrip += "<span class=\"docref\"["+Resources.Strings.Notices_clasifiated_list + "]</span>";
            string szBreadCrumb = "";
            ...
            cls.ClsHtml.FncHtmlBreadcrumbAdd(ref szBreadCrumb,szDescrip, "");
            cls.ClsHtml.FncHtmlBreadcrumbEnd(ref szBreadCrumb);
            return szBreadCrumb;
        }

        public String FncHtmlLastnotices(bool bRebuild, UInt16 iCount=20)
        {
            string szFileCache = cls.ClsGlobal.DirCache + "last_notices.html";
            ...
            {
                String szHtml = FncHtmlBldLastnoticesCache(iCount);
        ...
        private string FncHtmlBldLastnoticesCache(UInt16 Icount)
        {

            string szSqlSelect = "select loc_title as Url, `tdoclng_notices`.`DocId` AS `DocId`,`tdoclng_notices`.`DocLngId` AS `DocLngId`,`tdoclng_notices`.`Title` AS `Title`,`tdoc`.`DocTypeGroup` AS `DocTypeGroup`,concat('/a_files/doc_thumbnails/',`tdoc`.`DocImgThumbnail`) AS `DocImgThumbnail`,`tdoc`.`DocDateUpdate` AS `DocDateUpdate`,`tdoc`.`DocStdValLow` AS `DocStdValLow`,`tdoc`.`DocStdValHig` AS `DocStdValHig`,`tdoc`.`DocStdValMed` AS `DocStdValMed`  from (`tdoclng_notices` left join `tdoc` on((`tdoclng_notices`.`DocId` = `tdoc`.`DocId`))) INNER JOIN tdoclng_sitemap  ON `tdoclng_notices`.`DocId` = `tdoclng_sitemap`.`DocId` AND   `tdoclng_notices`.`DocLngId` = `tdoclng_sitemap`.`DocLngId`  where (`tdoc`.`DocIsActive` = 1) order by `tdoclng_notices`.`DocId` desc limit " + Icount.ToString();


            //string szSqlSelect = "Select `DocId`,`DocLngId`,`Title`,`DocImgThumbnail`,`DocDateUpdate`,  `DocStdValLow`, `DocStdValHig`, `DocStdValMed` from `wnoticelast20`";
            ...
            foreach (DataRow oRow in oDt.Rows)
            {
                ...
                sb.Append("<br/>" + cls.ClsHtml.FncHtmFlagLanguages(Convert.ToUInt64(oRow["DocId"].ToString()), "/notices/notice/"));
                sb.Append("<br/><br/></li>");
            }

            sb.Append("<li><b><a href=\"/" + ClsGlobal.LngIdThread + "/notices/notices-list\">" + Resources.Strings.Notices_clasifiated_list + "</a></b>");
        ...
        {
            string szFileCache = cls.ClsGlobal.DirCache + "last_notices_img.html";
        ...
        private string FncCache_GET_lastImgBld(ref UInt16 iCount)
        {
            ...
            string szSql = "";
            szSql += "SELECT `tdoclng_notices`.`DocId` as `DocId`, `tdoclng_notices`.`DocLngId` as `DocLngId`, `tdoclng_notices`.`Abstract` as `Abstract`,  `tdoclng_notices`.`Title` as `Title`, `tdoclng_notices`.`abstract` as `abstract`, `tdoc`.`DocTypeGroup`, `tdoc`.`DocTypeGroupSub` , `tdoc`.`DocAuthors` ,";
            ...
            szSql += " FROM `tdoclng_notices` LEFT JOIN `tdoc` ON `tdoclng_notices`.`DocId` = `tdoc`.`DocId`  ";
            szSql += " Where `tdoc`.`DocIsActive`='1'     GROUP BY DOCID order by  tdoc.DocDateUpdate desc limit " + iCount.ToString(); ;
            MySqlConnection oCnn = new MySqlConnection(ClsGlobal.Connection_MARIADB);
            ...
            System.Text.StringBuilder sb = new System.Text.StringBuilder();
            sb.Append("<h2 class=\"h2_reverse\">" + Resources.Strings.Last_Update_Notices + "</h2>");
            sb.Append("\n");
            string szUrl = "";
            foreach (DataRow oRow in oDt.Rows)
            {
               ...
               sb.Append("\n<h2><a href=\"" + szUrl + "\">" + oRow["Title"].ToString() + "</a></h2>");
                ...
                sb.Append("<br/>" + cls.ClsHtml.FncHtmFlagLanguages(Convert.ToUInt64(oRow["DocId"].ToString()), "/notice/article/") + "\"</span>");
                sb.Append("<hr />");
            }
