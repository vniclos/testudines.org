...
namespace testudines.cls.bbdd
{
    ...
    //<//summary>
    public class ClsReg_tdoclng_foods
...

    {
        ...
        //---------------------------------------------------
        //public ClsReg_tdoclng_foods(string szSqlConnectionString)
        //{
        public ClsReg_tdoclng_foods()
        {
        ...
        public void FncSqlBuildCommands()
        {
            ...
            //-------------- COMANDO SELECT  ---------------------------------------
            //----------------------------------------------------------------------
            szSql = "SELECT * FROM tdoclng_foods  ";
            szSql += " WHERE ";
            szSql += "(DocId=@DocId )";
            ...
            //-------------- COMANDO INSERT ----------------------------------------
            //----------------------------------------------------------------------
            szSql = "INSERT INTO tdoclng_foods";
            ...
            //-------------- COMANDO UPDATE ---------------------------------------
            //----------------------------------------------------------------------
            szSql = "UPDATE tdoclng_foods SET ";
            ...
            //-------------- COMANDO DELETE  ---------------------------------------
            //----------------------------------------------------------------------
            szSql = "DELETE FROM tdoclng_foods  ";
            szSql += " WHERE ";
            szSql += "(DocId=@DocId )";
            szSql += " AND (DocLngId=@DocLngId )";
        ...
        {
            string m_urlTemplate = "/tortoises/foods/food/";
            string m_urlDocType = "food";
...

        {
            ...
            szHtml += "<div class=\"divboxArticle \"><h2>" + Resources.Strings.Food + "</h2>" + m_Body + "</div>\n";
        ...
        {
            string szFileShortName = "food_BC";
        ...
        private string FncHtmlBldCacheBreadCrumb()
        {
            ...
            cls.ClsHtml.FncHtmlBreadcrumbAdd(ref szBreadCrumb, Resources.Strings.Foods, "/foods/");
            cls.ClsHtml.FncHtmlBreadcrumbAdd(ref szBreadCrumb, Resources.Strings.Food + ": " + szDescrip, "");
        ...
        {
            string szFileCache = cls.ClsGlobal.DirCache + "last_foods.html";
            ...
            {
                String szHtml = FncHtmlBldLastfoodsCache(iCount);
        ...
        private string FncHtmlBldLastfoodsCache(UInt16 iCount)
        {

            string szSqlSelect = "select loc_title as Url, `tdoclng_foods`.`DocId` AS `DocId`,`tdoclng_foods`.`DocLngId` AS `DocLngId`,`tdoclng_foods`.`Title` AS `Title`,`tdoc`.`DocTypeGroup` AS `DocTypeGroup`,concat('/a_files/doc_thumbnails/',`tdoc`.`DocImgThumbnail`) AS `DocImgThumbnail`,`tdoc`.`DocDateUpdate` AS `DocDateUpdate`,`tdoc`.`DocStdValLow` AS `DocStdValLow`,`tdoc`.`DocStdValHig` AS `DocStdValHig`,`tdoc`.`DocStdValMed` AS `DocStdValMed`  from (`tdoclng_foods` left join `tdoc` on((`tdoclng_foods`.`DocId` = `tdoc`.`DocId`))) INNER JOIN tdoclng_sitemap  ON `tdoclng_foods`.`DocId` = `tdoclng_sitemap`.`DocId` AND   `tdoclng_foods`.`DocLngId` = `tdoclng_sitemap`.`DocLngId`  where (`tdoc`.`DocIsActive` = 1) order by `tdoclng_foods`.`DocId` desc limit "+iCount.ToString()+";";


            //string szSqlSelect = "Select `DocId`,`DocLngId`,`Title`,`DocImgThumbnail`,`DocDateUpdate`,  `DocStdValLow`, `DocStdValHig`, `DocStdValMed` from `wfoodlast20`";
            ...
            foreach (DataRow oRow in oDt.Rows)
            {
                ...
                szUrlDocId = "/" + pszDocLngId + "/tortoises/foods/food/" + pszDocId;
                ...
                sb.Append("<br/>" + cls.ClsHtml.FncHtmFlagLanguages(Convert.ToUInt64(oRow["DocId"].ToString()), "/tortoises/foods/food/"));
                sb.Append("<br/><br/></li>");
            }

            sb.Append("<li><b><a href=\"/" + ClsGlobal.LngIdThread + "/tortoises/foods/foods-list\">" + Resources.Strings.Foods_last_updates + "</a></b>");
        ...
        {
            string szFileCache = cls.ClsGlobal.DirCache + "last_foods_img.html";
        ...
        private string FncCache_GET_lastImgBld(ref UInt16 iCount)
        {
            ...
            string szSql = "";
            szSql += "SELECT `tdoclng_foods`.`DocId` as `DocId`, `tdoclng_foods`.`DocLngId` as `DocLngId`, `tdoclng_foods`.`Abstract` as `Abstract`,  `tdoclng_foods`.`Title` as `Title`, `tdoclng_foods`.`abstract` as `abstract`, `tdoc`.`DocTypeGroup`, `tdoc`.`DocTypeGroupSub` , `tdoc`.`DocAuthors` ,";
            ...
            szSql += " FROM `tdoclng_foods` LEFT JOIN `tdoc` ON `tdoclng_foods`.`DocId` = `tdoc`.`DocId`  ";
            ...
            UInt64 pDocid = 0;
            string pszDocid = "";
            string szUrlTemplate = "/tortoises/foods/food/";
            foreach (DataRow oRow in oDt.Rows)
            {
                pszDocid = oRow["DocId"].ToString();
                pDocid = Convert.ToUInt64(pszDocid);
